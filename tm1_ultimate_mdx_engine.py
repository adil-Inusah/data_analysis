...
class UltimateMdxEngine:
    ...
    def _build_advanced_set(self, config: Dict[str, Any]) -> MdxHierarchySet:
        ...
        # Scenario 3: Filter by Level (e.g., Level 0 leaves)
            ...
            axis_set = MdxHierarchySet.members(dim, hierarchy).filter_by_level(config["level"])
            
        # Default: Grab all members
        else:
            axis_set = MdxHierarchySet.tm1_subset_all(dim, hierarchy)
            
        # Layer: Dynamic Attribute Filtering
        ...
        # Layer: Dynamic Sorting (BASC, BDESC, ASC, DESC)
        if "sort_order" in config and config["sort_order"]:
            # e.g., sort_order = "BDESC"
            axis_set = axis_set.order(order=config["sort_order"])
            
        return axis_set

    def configure_columns(self, dimension_configs: List[Dict[str, Any]]):
        """Processes and cross-joins column configurations."""
        if not dimension_configs: return self
        axis_set = self._build_advanced_set(dimension_configs[0])
        for config in dimension_configs[1:]:
            axis_set = axis_set.cross_join(self._build_advanced_set(config))
    ...
    def configure_rows(self, dimension_configs: List[Dict[str, Any]], suppress_zeros: bool = True):
        ...
        # Loop to extract metadata text attributes as physical data columns
    ...
    def add_calculated_member(self, dimension: str, member_name: str, formula_mdx: str):
        """Injects a dynamic dynamic runtime equation directly into the query footprint."""
        calc_member = CalculatedMember.of(dimension, member_name, formula_mdx)
    ...
    def configure_context_filter(self, slices: List[Tuple[str, str, str]]):
        ...
        if slices:
            ...
            self.builder.add_tuple_to_where_clause(MdxTuple.of(*member_objects))
        return self
